...
「把整個倉庫打包成一個檔案，靠人搬過去」不是我們發明的。git 本身就有為這個場景設計的工具 —— [`git bundle`](https://git-scm.com/docs/git-bundle)，2010 年 Scott Chacon 的[介紹文](https://scottchacon.com/2010/03/10/bundles/)裡叫它 **sneakernet**：穿上球鞋走過去。原本針對的是斷網、隔離網段、無法存取共用伺服器的情況。

唯讀的 AI connector 是同一個問題的新版本：**讀得到，但沒有回程。**

那為什麼用 zip 而不用 bundle？實測比較（本專案的骨架，1 個 commit、2 個 tag）：

| | 大小 | tag | 工作區 | 內建驗證 | 使用者打得開 | 沒 commit 的檔案 |
|---|---|---|---|---|---|---|
| `git bundle` | **1.1 KB** | ✅ | ✅ clone 後還原 | ✅ `git bundle verify` | ❌ 不透明 | ❌ **不帶** |
| zip | 18.4 KB | ✅ | ✅ | 自行驗證 | ✅ | ✅ |

bundle 小十六倍，而且**沒有壓縮檔名這回事** —— 上面那個 UTF-8 旗標問題在 bundle 裡整個不存在，因為檔名活在 git 物件裡，本來就是 UTF-8。

本專案仍然選 zip，理由有兩個：

- **使用者打得開看。** bundle 是完全不透明的黑盒，zip 至少雙擊得開。這套東西的使用者不是工程師
- **bundle 只帶 commit 過的東西。** 任何被 `.gitignore` 排除的檔案都不會跟著走，而且是**無聲消失**。zip 帶走目錄裡的一切

在我們的規模下，18 KB 和 1 KB 沒有實際差別，所以拿大小換透明度和「不會少東西」是划算的。如果你的專案很大、或使用者確定不需要直接打開，bundle 是更好的選擇。
...
搜尋當下（2026 年 8 月）找不到針對「唯讀 connector + 對話式專案管理」的討論。相鄰的領域各自成熟，但都沒有涵蓋這個交集：

- **`git bundle` / sneakernet** —— 離線搬運倉庫的成熟做法，但假設兩端都有 git 和會用 git 的人
- **AI agent 狀態持久化** —— 討論很多，答案幾乎一律是資料庫或專用 workspace 服務，前提是可寫入
- **MCP 型的專案管理技能** —— Jira、Confluence 整合，全部假設有寫入權限
- **Claude Skill 的 zip 散佈** —— zip 在那裡是**技能的包裝格式**，不是專案狀態的載體

這個 repo 補的是那個交集：**已經連上了，但只能讀。**
